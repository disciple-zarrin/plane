...
export class WorkLogService extends APIService {
  ...
  async issueSummary(workspaceSlug: string, projectId: string, issueId: string) {
    return this.get(`/api/workspaces/${workspaceSlug}/projects/${projectId}/issues/${issueId}/worklogs-summary/`).then(
      (res) => res?.data
    );
  }
  ...
  async workspaceWorkLogs(
    ...
    const qs = q.toString();
    const path = qs ? `/api/workspaces/${workspaceSlug}/worklogs/?${qs}` : `/api/workspaces/${workspaceSlug}/worklogs/`;
