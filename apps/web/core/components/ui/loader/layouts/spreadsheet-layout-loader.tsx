...
export function SpreadsheetIssueRowLoader(props: { columnCount: number }) {
  return (
    <tr className="border-b border-subtle bg-surface-1">
      <td className="sticky start-0 z-[10] flex h-11 min-w-[28rem] items-center border-e-[0.5px] border-subtle bg-surface-1">
  ...
  return (
    <div className="horizontal-scroll-enable h-full w-full overflow-y-auto">
      <table>
        <thead>
          <tr>
            <th className="h-11 min-w-[28rem] animate-pulse border-e border-subtle bg-surface-2" />
            {range(10).map((index) => (
              <th key={index} className="h-11 w-full min-w-[8rem] animate-pulse border-e border-subtle bg-surface-2" />
            ))}
          </tr>
        </thead>
        <tbody>
